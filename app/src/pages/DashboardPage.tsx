...
export default function DashboardPage() {
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const fileMessage = `[Uploaded File: ${data.file.originalName}]`;
  ...
  const handleSendMessage = async (e?: FormEvent) => {
    ...
    // Use current conversation ID if messages exist, otherwise undefined (new chat)
    const currentConversationId = messages.length > 0 ? messages[0].conversation_id : undefined;
    ...
    try {
      ...
      if (!currentConversationId && data.conversationId) {
        fetchConversations();
        // Update local messages with the new conversation ID if needed
      ...
      >
        <div className="p-6">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-4 py-3 bg-black text-white hover:bg-gray-800 rounded-xl text-sm font-medium transition-all shadow-sm hover:shadow"
          >
        ...
        <div className="p-4 border-t border-gray-100 space-y-1">
          <button className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-500 hover:bg-gray-50 rounded-lg transition-colors">
